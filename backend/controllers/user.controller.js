...
export const followUnfollowUser = async (req, res) => {
  try {
    ...
    if (isFollowing) {
      ...
      res.status(200).json({ message: "User unfollowed successully" });
    } else {
      // follow user
    ...
    console.log("Error in follow and unfolllow user", err.message);
    res.status(500).json({ error: err.message });
  }
};
...
export const updateUser = async (req, res) => {
  ...
  try {
    ...
    console.log("Error in update users", err.message);
    res.status(500).json({ error: err.message });
  }
};
